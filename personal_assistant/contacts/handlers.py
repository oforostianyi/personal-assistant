...
CTX_ROOT       : `contacts` — enter the module.
CTX_CONTACTS   : list / sort / find / filter / new.
CTX_CONTACT    : show / notes / delete (notes is a stub until Stage 5).

`create_contact_from_input` is the shared creation path for the `new <name>`
command and the entity-enter fallback, so the logic is not duplicated.
"""
...
from personal_assistant.core.registry import (
    CTX_CONTACT, CTX_CONTACTS, CTX_NOTES, CTX_ROOT, command,
)
from personal_assistant.ui.views import (
    render_contact_card, render_contacts_table, render_notes_table,
)


# All interactive `edit` menus read additional input through input() in the
# handler body. The hw-08 "I/O only in main.py" rule is intentionally relaxed
# here: otherwise every two-step edit would require separate commands
# (`edit-name-prompt`, `edit-name-set`), which would make the UX worse.
def _ask(prompt: str) -> str | None:
    """Read one line; return None on EOF/Ctrl-C (cancellation)."""
    try:
        return input(prompt)
...
def create_contact_from_input(query: str, state) -> str:
    ...
    card is rendered by the dispatcher's auto-show after this function returns
    and `state.entity_key` has changed.
    """
    record = Record(query)
...
def contact_notes(_args, state):
    ...
    if record is None:
        raise KeyError(state.entity_key)
    # NotesBook.find_by_uuid is introduced in Stage 5.
    ...
    from personal_assistant.ui.views import render_notes_table
...
def _phone_submenu(record: Record) -> str:
    for _ in range(4):
        ...
        if 1 <= n <= len(record.phones):
            ...
            # else: back / unrecognized -> loop again.
            continue
        print(f"Invalid choice '{choice}'. Try again.")
...
def edit_contact(args, state):
    ...
    if not args:
        # Main menu: reprompt up to 3 times on invalid input (4 attempts total).
        for _ in range(4):
...
def _weekday_name(date_str: str) -> str:
    """Return the English weekday name for `date_str` in DD.MM.YYYY format.

    Locale-independent: do not rely on `strftime("%A")`, which depends on the
    system locale.
    """
